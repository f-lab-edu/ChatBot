...
public class DataSourceTest {
    ...
    void masterDataSourceTest(@Qualifier("MASTER") final DataSource masterDataSource) {
        ...
        String driverClassName = environment.getProperty(
            "spring.datasource.hikari.master.driver-class-name");
        Boolean readOnly = Boolean.valueOf(
            environment.getProperty("spring.datasource.hikari.master.read-only"));
    ...
    void slaveDataSourceTest(@Qualifier("SLAVE") final DataSource masterDataSource) {
        ...
        String driverClassName = environment.getProperty(
            "spring.datasource.hikari.slave.driver-class-name");
        Boolean readOnly = Boolean.valueOf(
            environment.getProperty("spring.datasource.hikari.slave.read-only"));
    ...
    void readOnlyTest()
        ...
        // when
        Method determineCurrentLookupKey = DynamicRoutingDataSource.class.getDeclaredMethod(
            Test_Method_Name);
        determineCurrentLookupKey.setAccessible(true);

        DataSourceType result = (DataSourceType) determineCurrentLookupKey.invoke(
