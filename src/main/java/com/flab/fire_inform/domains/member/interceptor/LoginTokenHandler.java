...
public class LoginTokenHandler implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (accessToken != null && jwtProvider.validationToken(accessToken)) {
            ...
            LoginRequest loginRequest = (LoginRequest) session.getAttribute(
                request.getHeader("REFRESH_TOKEN_KEY"));
            ...
            if (refreshToken != null && jwtProvider.validationToken(refreshToken)) {
                String newAccessToken = jwtProvider.createAccessToken(loginRequest.getId(),
                    loginRequest.getPassword());
                response.setHeader(ACCESS_TOKEN, newAccessToken);
                log.info("[Prehandler] :::::REFRESH_TOKEN 통과 ---> NewAccessToken ={} ",
                    newAccessToken);
                return true;
            }
