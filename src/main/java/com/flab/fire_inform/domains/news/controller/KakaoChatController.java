...
import com.flab.fire_inform.domains.news.dto.newsList.Button;
import com.flab.fire_inform.domains.news.dto.newsList.ListCard;
...
public class KakaoChatController {
    ...
    @RequestMapping(value = "/api/news/{domain}", method = {RequestMethod.POST,
    ...
    public SkillResponse newsListAPI(@PathVariable(required = false) String domain
        ...
        // header setting
        String date =
            DateTimeFormatter.ofPattern("MM월 dd일(E) ").format(LocalDateTime.now()) + " 경제 뉴스입니다.";
    ...
    @RequestMapping(value = "/api/stock", method = {RequestMethod.POST,
        RequestMethod.GET}, headers = {"Accept=application/json"})
    public SkillResponse getStockInfo(@RequestBody(required = false) Map<String, Object> params)
        throws IOException {
        ...
        // header setting
        String date = DateTimeFormatter.ofPattern("MM월 dd일(E) ").format(LocalDateTime.now())
            + stockInformation.getName() + " 주식";
        ListItem header = ListItem.builder(date).build();

        // 본문 아이템 리스트
        List<ListItem> items = new ArrayList<>();
        items.add(ListItem.builder(stockInformation.getPrice())
            .description(stockInformation.getHowDifferenttWithYesterday()).build());
    ...
    private SkillResponse listCardSkillResponse(ListItem header, List<ListItem> items,
        List<Button> buttons) {
