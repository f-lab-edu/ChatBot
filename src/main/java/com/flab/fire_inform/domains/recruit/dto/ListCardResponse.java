...
public class ListCardResponse implements CommonResponse {
    ...
    private static final Map<String, String> logoUrlMap = Map.of("kakao",
        "https://t1.kakaocdn.net/kakaocorp/corp_thumbnail/Kakao.png", "naver",
        "https://blog.kakaocdn.net/dn/XEAGa/btqB10c0bQ1/RZk86MYViwrP4OaBdpfhn0/img.png");
    ...
    public static ListCardResponse of(List<Recruit> recruits) {
        ...
        while (currentIndex < totalCount) {
            ...
            for (int i = currentIndex; i < totalCount; i++) {
                ...
                Item item = new Item(recruit.getTitle(), sb.toString(),
                    logoUrlMap.get(recruit.getCompany()), new Link(recruit.getLink()));
                items.add(item);
                if (i == totalCount - 1 || i - currentIndex == 4) {
                    outputs.add(new Output(new ListCard(
                        new Header(now + " 신규 채용 공고 (" + currentPage++ + "/" + totalPage + ")"),
                        items)));
                    currentIndex = i + 1;
                    break;
                }
